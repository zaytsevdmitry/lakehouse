...
public class ScenarioActInstanceService {
	...
	public ScenarioActInstanceService(ScheduleInstanceLastRepository scheduleInstanceLastRepository,
	...
	public List<ScheduleScenarioActInstance> getScenario(ScheduleInstance scheduleInstance) {
		return new ArrayList<>(scenarioActRepository.findByScheduleName(scheduleInstance.getSchedule().getName())
	...
	@Transactional
	public void SaveScenario(ScheduleInstance scheduleInstance,
	...
	public void runNewScenarios() {
		scheduleInstanceLastRepository
				.findByScheduleEnabled().stream().filter(scheduleInstanceLast -> scheduleInstanceLast
						.getScheduleInstance().getStatus().equals(Status.Schedule.NEW.label))
				.forEach(scheduleInstanceLast -> {
					try {
						SaveScenario(scheduleInstanceLast.getScheduleInstance(),
