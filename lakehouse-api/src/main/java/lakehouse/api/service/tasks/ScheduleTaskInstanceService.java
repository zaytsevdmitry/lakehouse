...
/*
    private ScheduleTaskInstance mapToScheduleTaskInstance(TaskTemplate taskTemplate) {
    ...
    public void entryTasksQueued(ScheduleInstance scheduleInstance){
        ...
        entryPoints.removeAll(
                scheduleTaskInstanceDependencies
                        .stream()
                        .map(ScheduleTaskInstanceDependency::getScheduleTaskInstance)
                        .toList());
    ...
    private ScheduledTaskDTO mapScheduledTaskDTO(ScheduleTaskInstance scheduleTaskInstance){
        ...
        result.setExecutionModuleArgs(null); //todo stub
        result.setDescription(
                String.format(
                        "Schedule %s time %s task %s",
                        scheduleTaskInstance.getScheduleInstance().getSchedule().getName(),
                        scheduleTaskInstance.getName(),
                        DateTimeUtils.formatDateTimeFormatWithTZ(scheduleTaskInstance.getScheduleInstance().getTargetExecutionDateTime())));

        return result;
    }*/
/*    private List<ScheduleTaskInstance> getTaskInstances(ScheduleInstance scheduleInstance){
    ...
    public void runNewTasks() {
        scheduleInstanceLastRepository
                ...
                .filter(scheduleInstanceLast -> scheduleInstanceLast
                        .getScheduleInstance()
                        .getStatus()
                        .equals(Status.Schedule.READY_ACTS.label))
...
/*    public ScheduledTaskDTO takeTaskToExecute(TaskExecutionServiceGroup taskExecutionServiceGroup, String serviceId){
        scheduleTaskInstanceRepository.findQueuedBy(taskExecutionServiceGroup.getName());
    }*/


    /*private int getIndexByName(List<String> strings, String string){
        for (int i=0; i < strings.size(); i++){
            if (strings.get(i).equals(string)) return i;
        }
        return -1;
    }
    private void saveTasks(Schedule schedule, List<TaskTemplate> taskTemplates, List<DagTaskEdge> dagTaskEdges){
        ...
        Set<String> verticesHS = new HashSet<>();
        taskTemplates.forEach(taskDTO -> verticesHS.add(taskDTO.getName()));

        // diff between task list and "to" take entry vertices
        List<String> entryPoints = new ArrayList<>(verticesHS);
        entryPoints.removeAll(dagTaskEdges.stream().map(dagTaskEdge -> dagTaskEdge.getToTaskTemplate().getName()).toList());
        //todo debug
        entryPoints.forEach(System.out::println);

        // union entry vertices in zero start node
        List<String> vertices = new ArrayList<>();
        vertices.add(0,"begin");
        vertices.addAll(verticesHS);
        //todo debug
        vertices.forEach(System.out::println);

        LinkedList<Integer>[] edges = new LinkedList[vertices.size()];
