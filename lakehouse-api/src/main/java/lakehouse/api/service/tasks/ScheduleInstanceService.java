...
public class ScheduleInstanceService {
    ...
    public ScheduleInstanceService(
            ...
            ScenarioActEdgeRepository scenarioActEdgeRepository, Mapper mapper,
    ...
    private ScheduleInstance newScheduleInstance(ScheduleInstanceLast scheduleInstanceLast){
        ...
        try {
            scheduleInstance
                    .setTargetExecutionDateTime(
                            DateTimeUtils.getNextTargetExecutionDateTime(
                                    scheduleInstanceLast
                                            .getSchedule()
                                            .getIntervalExpression(),
                                    lastTargetExecutionDate));
        } catch (CronParceErrorException e) {
            logger.warn(e.getMessage());
            throw new TransactionException(
                    String.format(
                            "Error when try to set TargetExecutionDateTime of %s",
                            scheduleInstance.getSchedule().getName()),
                    e);
        }
    ...
    private List<ScheduleScenarioActInstance> getScheduleScenarioActInstances(
            ScheduleInstance scheduleInstance,
            List<ScenarioAct> scenarioActs){
        return scenarioActs
                .stream().map(scenarioAct -> mapper.mapToScheduleScenarioActInstance(scenarioAct,scheduleInstance))
                .toList();
    }

    private List<ScheduleTaskInstance> getTaskInstancesByAct(
            Map<String,ScheduleScenarioActInstance> actInstanceMap,
            Map<ScenarioAct,Map<String,TaskTemplate>> scenarioTaskTemplateMap

    ){
        return scenarioTaskTemplateMap.keySet()
                .stream()
                .flatMap(scenarioAct -> scenarioTaskTemplateMap.get(scenarioAct).values()
                            .stream()
                            .map(taskTemplate ->  mapper.mapToScheduleTaskInstance(taskTemplate,actInstanceMap.get(scenarioAct.getName()))))
                .toList();
    }
    ...
    private List<ScheduleScenarioActInstanceDependency> getScheduleScenarioActInstanceDependencies(
        ...
        return scenarioActEdgeRepository
    ...
    private List<ScheduleTaskInstanceDependency> getScheduleTaskInstanceDependencies(
            List<ScenarioAct> scenarioActs,
        ...
        scenarioActs.forEach(scenarioAct -> {
            ...
            scheduleTaskInstances
                    .stream()
                    .filter(sti -> sti.getScheduleScenarioActInstance()
                            .getName()
                            .equals(scenarioAct.getName()))
        ...
        List<ScheduleTaskInstanceDependency> scheduleTaskInstanceDependencies =
                 taskInstanceMapByAct.keySet()
                .stream()
                .flatMap(sa ->
                    taskTemplateEdgeRepository
                            .findByScenarioTemplateName(sa.getScenarioActTemplate().getName())
                            .stream()
                            .map(taskTemplateEdge -> mapper
                                    .mapToScheduleTaskInstanceDependency(
    ...
    private Map<ScenarioAct,Map<String,TaskTemplate>> getScenarioTaskTemplateMap(List<ScenarioAct> scenarioActs) {
        ...
        scenarioActs.forEach(scenarioAct ->   {
            ...
            taskTemplateRepository
                    .findByScenarioTemplateName(scenarioAct.getScenarioActTemplate().getName())
    ...
    private void save(ScheduleInstanceLast scheduleInstanceLast){
        ...
        transactionTemplate.execute(status -> {
            try {
                ...
                ScheduleInstance scheduleInstance = scheduleInstanceRepository.save(newScheduleInstance(scheduleInstanceLast));

                List<ScheduleScenarioActInstance> scheduleScenarioActInstances =
                ...
                List<ScheduleScenarioActInstanceDependency> scheduleScenarioActInstanceDependencies =
                ...
                List<ScheduleTaskInstanceDependency> scheduleTaskInstanceDependencies =
                        scheduleTaskInstanceDependencyRepository.saveAll(
                                getScheduleTaskInstanceDependencies(scenarioActs,scheduleTaskInstances));
