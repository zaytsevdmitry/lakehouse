...
export default function AdminView({ session, profile, onNotice }) {
  ...
  const loadWorkspaces = useCallback(async () => {
    const data = await api('/v1_0/admin/workspaces', { token });
    setWorkspaces(Array.isArray(data) ? data : []);
  }, [token]);

  const loadTtl = useCallback(async () => {
    setTtl(await api('/v1_0/admin/settings/cleanup-ttl-hours', { token }));
  }, [token]);

  const loadLogs = useCallback(async () => {
    const data = await api('/v1_0/admin/sync-logs?limit=200', { token });
  ...
  const forceDelete = async (ws) => {
    ...
    setBusy(true);
    try {
      await api(`/v1_0/admin/workspaces/${encodeURIComponent(ws.id)}`, { method: 'DELETE', token });
