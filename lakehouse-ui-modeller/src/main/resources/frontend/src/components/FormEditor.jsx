...
import { ColumnPickerModal, ConstraintPickerModal, ScriptKeyPreviewModal } from './Pickers';
...
export default function FormEditor({
  ...
  return (
    <FormContext.Provider value={{ uniqueByField, rootDoc: root, dataSetSummaryProvider, scriptSummaryProvider }}>
...
function NestedTab({ field, value, onChange, readOnly, rootDoc, onBaseChange }) {
  if (field.type === 'dag') {
    return (
...
function ScalarField({ field, value, onChange, readOnly, keyNameEditable = true }) {
  ...
  const { uniqueByField, rootDoc, scriptSummaryProvider } = React.useContext(FormContext);
        ...
        return (
          <div className="box-row row-stack" key={key}>
            ...
            {isObjectMap ? (
              ...
              <ScalarField
                field={valueChildren[0] || { name: 'value', type: 'string', label: 'value' }}
                value={entry}
                onChange={(next) => updateEntry(key, next)}
                readOnly={readOnly}
              />
