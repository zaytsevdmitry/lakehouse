...
The service opens JDBC connections through the `JdbcConnectionFactory` of `lakehouse-task-executor-api`.
With the `lakehouse-credential-providers-jdbc` jar on the classpath, a JDBC datasource `service.properties`
...
`lakehouse-task-executor-svc` is protected with OAuth 2.0 / OIDC and uses Keycloak as the identity provider (realm `lakehouse`). Spring Security is configured as an **OAuth2 resource server**: every request must carry a valid JWT issued by the realm, otherwise the service returns `401`.
