# lakehouse-credential-providers-spark

Spark-specific credential providers for Apache Spark 3.5.x that dynamically retrieve secrets from **OpenBao / HashiCorp Vault** and **Yandex Cloud Lockbox** at task execution time on both Driver and Executors — without Spring Framework.

The project is split into two modules:

| Module | Contents |
|--------|----------|
| `lakehouse-credential-providers-jdbc` | Shared, Spark-independent code: the `SecretProvider` SPI, `VaultHttp` / `LockboxHttp` HTTP clients, `SecretCache`, the JDBC providers (`BaoJdbcSecretProvider`, `YcLockboxJdbcSecretProvider`), `LakehouseSecurityContext` and the `SecretResolver` helper. Reused by lakehouse services (e.g. `lakehouse-task-executor-api`). |
| `lakehouse-credential-providers-spark` | Spark-specific code: the S3 providers (`BaoS3CredentialsProvider`, `YcLockboxS3CredentialsProvider`) and `LakehouseSecureJDBCTableCatalog`. Depends on the shared module. |
...
```bash
mvn clean package -pl lakehouse-credential-providers-jdbc,lakehouse-credential-providers-spark -am -DskipTests
```

### Deploying to Spark

Copy **both** JARs to every node's `spark/jars/` directory, **or** pass them via `--jars` at submission:

```bash
spark-submit \
  --jars lakehouse-credential-providers-jdbc-0.8.0.jar,lakehouse-credential-providers-spark-0.8.0.jar \
  ...
```

Both JARs have **zero transitive runtime dependencies** — all runtime dependencies (Spark, Hadoop, AWS SDK, Jackson) are already on the Spark classpath. In the demo Docker stack the two JARs are copied into the image by `docker/lakehouse-spark-aws/build.bash`.
...
### Using the providers in lakehouse services (JDBC path)

The shared module is also used by non-Spark lakehouse services. The JDBC connection factory
of `lakehouse-task-executor-api` (`JdbcConnectionFactory`) applies the same option contract:
when a `ServiceDTO.properties` map contains `secretProvider`, the password is resolved before
`DriverManager.getConnection(...)` and the security keys are stripped from the JDBC properties.

```properties
# Service.properties of a JDBC DataSource (storage of ServiceDTO.properties in lakehouse-config-svc)
url            jdbc:postgresql://postgres:5432/lakehouse_db
user           lakehouse_user
...
Supported keys: `secretProvider`, `secret-key` (combined `path:key`), and provider-specific
`vault-url`, `vault-role`, `vault-k8s-auth-path`, `secret-id`, `secret-version`. All of them are
removed before the options are passed to the JDBC driver. When `secretProvider` is absent the map
is passed through unchanged, so existing configurations keep working as-is.
