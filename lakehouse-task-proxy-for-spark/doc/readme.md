...
`lakehouse-task-proxy-for-spark` is protected with OAuth 2.0 / OIDC and uses Keycloak as the identity provider (realm `lakehouse`). Spring Security is configured as an **OAuth2 resource server**: every request must carry a valid JWT issued by the realm, otherwise the service returns `401`.
...
Whitelisted paths (no token required): `/healthz`, `/readyz`, `/actuator/**` (including Prometheus metrics), `/v3/api-docs/**`, `/swagger-ui/**`.
