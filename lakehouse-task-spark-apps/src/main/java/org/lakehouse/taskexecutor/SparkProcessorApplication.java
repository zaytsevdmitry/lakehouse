...
public class SparkProcessorApplication {
    ...
    public static void main(String[] args) throws TaskConfigurationException, JsonProcessingException, TaskFailedException {
       ConfigurableApplicationContext applicationContext = SpringApplication.run(SparkProcessorApplication.class, args);

        if (args.length >= 1) {
            logger.info(args[0]);
            ScheduledTaskLockDTO scheduledTaskLockDTO = jsonToConf(args[0],ScheduledTaskLockDTO.class);
            ...
            ProcessorBody body = (ProcessorBody) applicationContext.getBean(scheduledTaskLockDTO.getScheduledTaskEffectiveDTO().getTaskProcessorBody());
            BodyParam bodyParam = sparkProcessorBodyParamFactory.buildSparkProcessorBodyParameter(scheduledTaskLockDTO);
            body.run(bodyParam);
        } else {
