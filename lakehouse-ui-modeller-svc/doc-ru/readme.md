...
Backend устроен как декларативный «коммутатор»: каждая стратегия выбирается через одно дерево конфигурации `lakehouse.modeller.*` (см. *Конфигурация*). SPA аутентифицируется напрямую в Keycloak (PKCE), и каждый запрос `/v1_0` несёт Bearer JWT, проверяемый ресурс-сервером.
...
- **Configuration** (`config`) — `ModellerProperties` (`@ConfigurationProperties(prefix = "lakehouse.modeller")`, обнаруживается через `@ConfigurationPropertiesScan`), `ModellerConfiguration` (бины стратегий: хранилище, VCS, менеджер воркспейсов, сервисы, JWT-декодер, CORS) и `SecurityConfig` (безопасность без состояния, Bearer JWT).
...
- коммутатор `ModellerProperties`, бины `ModellerConfiguration` и `SecurityConfig`;
...
Все параметры находятся в одном дереве `lakehouse.modeller.*` (`src/main/resources/application.yml`); каждое значение переопределяется переменной окружения (`LAKEHOUSE_*`) или аргументом командной строки `--lakehouse.modeller.*=`. Связывание декларативное, через `@ConfigurationPropertiesScan`, — сервис стартует только когда выбранные стратегии полностью сконфигурированы (например, отсутствие системной учётной записи останавливает запуск с явной ошибкой).
...
java -jar lakehouse-ui-modeller.jar \
  --lakehouse.modeller.vcs-provider=local-git \
  --lakehouse.modeller.git.remote-url=/srv/git/lakehouse-metadata.git \
  --lakehouse.modeller.git.branch-main=main \
  --lakehouse.modeller.auth-strategy=jwt-rbac \
  --lakehouse.modeller.vcs-system-account.auth-type=token \
  --lakehouse.modeller.vcs-system-account.token=<git-pat-или-oauth2-token> \
  --lakehouse.modeller.vcs-system-account.username=<пользователь> \
  --lakehouse.modeller.security.oauth2.resourceserver.jwt.issuer-uri=http://localhost:8080/realms/lakehouse
```
...
Modeller работает как **BFF** (OAuth 2.0 authorization code + PKCE): браузер перенаправляется в Keycloak, код обменивается на стороне backend'а, и сохраняется аутентифицированная сессионная cookie (`JSESSIONID`, HttpOnly). Это же приложение дополнительно валидирует Bearer JWT как стандартный OAuth2 Resource Server для доверенных клиентов. Realm-роли (`realm_access.roles` / роли клиента), прикреплённые к principal — из ID token / userinfo при входе и из JWT-claims на bearer-пути — приводятся к матрице RBAC:
...
Публичные пути (аутентификация не требуется): `/`, `/index.html`, `/assets/**`, `/favicon.ico`, `/vite.svg`, `/manifest.json`, `/robots.txt`, `/v1_0/auth/config`, а также любые корневые `*.css`/`*.js`/`*.png`/`*.jpg`/`*.svg`.
...
|---|---|
| Realm | `lakehouse` |
| BFF-клиент | `lakehouse-ui-modeller-client` (конфиденциальный, authorization-code + PKCE); redirect `http://localhost:8081/login/oauth2/code/keycloak`, web origin `http://localhost:8081` |
| JWK для ресурс-сервера | из `KEYCLOAK_ISSUER_URI` (issuer должен быть доступен backend'у) |
| Роли → claims | у BFF-клиента должен быть client-level маппер realm-ролей, публикующий `realm_access.roles` в **ID token и userinfo** (`id.token.claim`/`userinfo.token.claim`), а не только в access token — principal BFF строится из этих claims |
