...
The **Modeller** service: a workspace-centric **YAML metadata editor** for declarative lakehouse configurations. It provides a single-page React application, a Spring Boot backend that owns the server-side workspaces (checkout/seed, edit, review/submit), and the VCS integration that pushes metadata changes to the central repository.
...
`lakehouse.modeller.*` configuration tree (see *Configuration*). The SPA authenticates directly
...
- **Configuration** (`config`) — `ModellerProperties` (`@ConfigurationProperties(prefix = "lakehouse.modeller")`, discovered via `@ConfigurationPropertiesScan`), `ModellerConfiguration` (strategy beans: storage, VCS, workspace manager, services, JWT decoder, CORS) and `SecurityConfig` (stateless bearer-token security).
...
- the `ModellerProperties` switchboard, `ModellerConfiguration` beans and `SecurityConfig`;
...
All parameters live in a single `lakehouse.modeller.*` tree (`src/main/resources/application.yml`), every value overridable with an environment variable (`LAKEHOUSE_*`) or a `--lakehouse.modeller.*=` command-line argument. Binding is declarative via `@ConfigurationPropertiesScan` — the service boots only when the selected strategies are fully configured (a missing system account, for example, fails fast at startup).
...
java -jar lakehouse-ui-modeller.jar \
  ...
  --lakehouse.modeller.vcs-system-account.token=<git-pat-or-oauth2-token> \
  --lakehouse.modeller.vcs-system-account.username=<user> \
...
The modeller runs as a **BFF** (OAuth 2.0 authorization code + PKCE): the browser is redirected to Keycloak and the backend exchanges the code server-side, storing an authenticated session cookie (`JSESSIONID`, HttpOnly). The same application also validates bearer JWTs as a standard OAuth2 Resource Server for trusted clients. Realm roles (`realm_access.roles` / client roles) attached to the principal — from the ID token / user-info on the login flow and from the JWT claims on the bearer flow — are normalized to the RBAC matrix:
...
Whitelisted paths (no authentication required): `/`, `/index.html`, `/assets/**`, `/favicon.ico`, `/vite.svg`, `/manifest.json`, `/robots.txt`, `/v1_0/auth/config`, and any top-level `*.css`/`*.js`/`*.png`/`*.jpg`/`*.svg`.
...
|---|---|
| Realm | `lakehouse` |
| BFF client | `lakehouse-ui-modeller-client` (confidential, authorization-code + PKCE); redirect `http://localhost:8081/login/oauth2/code/keycloak`, web origin `http://localhost:8081` |
| Resource-server JWK | from `KEYCLOAK_ISSUER_URI` (issuer must be reachable by the backend) |
| Roles → claims | a client-level realm-role mapper on the BFF client must publish `realm_access.roles` into the **ID token and user-info** (`id.token.claim`/`userinfo.token.claim`), not only the access token — the BFF principal is built from those claims |
