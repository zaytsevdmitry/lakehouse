...
public class FileLoader {
    ...
    public Map<String, DataSourceDTO> loadAllDataSources() throws IOException {
        Map<String, DataSourceDTO> result = new HashMap<>();
        for (String filename: getFilenames(dataSourcesDir)) {
            DataSourceDTO dataSourceDTO = loadDataSourceDTO(filename);
            result.put(dataSourceDTO.getKeyName(), dataSourceDTO);
        }
        return result;
    }

    public ScenarioActTemplateDTO loadScenarioDTO(String name) throws IOException {
        return objectMapper
                .readValue(
                        new File(
                                String.format(
                                rootPath.concat("/scenario-act-templates/%s.json"),name)), ScenarioActTemplateDTO.class);
    }
