...
import org.lakehouse.ui.modeller.dto.FieldSchema;
...
/**
 * Reflection-driven form schema generator. For every {@link YamlMetadataKind} it resolves the
...
public class SchemaService {
    ...
    public SchemaService() {
        for (YamlMetadataKind kind : YamlMetadataKind.values())
            schemas.put(YamlMetadataKind.normalize(kind.yamlValue()), build(kind));
    }

    public KindSchema schema(String kind) {
        return schemas.get(YamlMetadataKind.normalize(kind));
    }
    ...
    private KindSchema build(YamlMetadataKind kind) {
