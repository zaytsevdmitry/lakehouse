...
import DataLineageDiagramEditor from './DataLineageDiagramEditor';
...
export default function EditorView({ profile, workspace, onBack, onNotice }) {
  ...
  const loadDirs = useCallback(async () => {
    const data = await api(`/api/workspaces/${encodePath(wsId)}/dirs`);
  ...
  useEffect(() => {
    (async () => {
      try {
        const [schemaData] = await Promise.all([api('/api/schema'), loadTree(), loadDirs()]);
  ...
  const save = async () => {
    ...
    setBusy(true);
    try {
      const resp = await api(`/api/workspaces/${encodePath(wsId)}/files/${encodePath(selected.path)}`, {
        method: 'PUT',
  ...
  const createFile = async ({ kind, keyName }) => {
    ...
    setBusy(true);
    try {
      const resp = await api(`/api/workspaces/${encodePath(wsId)}/files`, {
        method: 'POST',
  ...
  const removeFile = async () => {
    ...
    setBusy(true);
    try {
      await api(`/api/workspaces/${encodePath(wsId)}/files/${encodePath(selected.path)}`, {
        method: 'DELETE',
      });
  ...
  const submitReview = async ({ comment, commitMessage }) => {
    setBusy(true);
    try {
      const resp = await api(`/api/vcs/review/${encodePath(wsId)}`, {
        method: 'POST',
  ...
  const renameFile = async (newName) => {
    ...
    setBusy(true);
    try {
      const resp = await api(`/api/workspaces/${encodePath(wsId)}/files/rename`, {
        method: 'POST',
  ...
  const deleteDir = async () => {
    ...
    setBusy(true);
    try {
      await api(`/api/workspaces/${encodePath(wsId)}/dirs/${encodePath(deleteDirTarget)}`, {
        method: 'DELETE',
      });
  ...
  const revert = async () => {
    ...
    setBusy(true);
    try {
      const resp = await api(`/api/vcs/workspace/${encodePath(wsId)}/restore`, {
        method: 'POST',
        body: { path: target },
      });
      await refreshTree();
