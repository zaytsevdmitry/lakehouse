...
public class GitOpsScheduler {
    ...
    public synchronized void sync() {
        ...
        try {
            initClient();
            vcsClient.pull();
            head = vcsClient.getCurrentCommitId();

            if (vcsSyncLogRepository.existsByCommitId(head)) {
                logger.debug("Commit {} already processed, skipping", head);
                return;
            }

            Optional<VcsSyncLog> lastSuccess = vcsSyncLogRepository.findFirstByStatusOrderBySyncDateTimeDesc(VcsSyncStatus.SUCCESS);
            ...
            logger.warn("VCS infrastructure failure, will retry: {}", e.getMessage());
        } catch (Exception e) {
