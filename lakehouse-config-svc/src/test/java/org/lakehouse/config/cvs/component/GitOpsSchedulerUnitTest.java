...
import org.lakehouse.config.cvs.yaml.CvsConfigParseException;
...
class GitOpsSchedulerUnitTest {
    ...
    @Mock
    private CvsSyncLogRepository cvsSyncLogRepository;
    ...
    void skipsCommitThatWasAlreadyProcessed() {
        when(cvsSyncLogRepository.existsByCommitId(HEAD)).thenReturn(true);
    ...
    void skipsWhenHeadEqualsLastSuccess() {
        when(cvsSyncLogRepository.existsByCommitId(HEAD)).thenReturn(false);
        when(cvsSyncLogRepository.findFirstByStatusOrderBySyncDateTimeDesc(CvsSyncStatus.SUCCESS))
    ...
    void appliesChangeSetAgainstLastSuccessBase() {
        when(cvsClient.getCurrentCommitId()).thenReturn(HEAD);
        when(cvsSyncLogRepository.existsByCommitId(HEAD)).thenReturn(false);
        when(cvsSyncLogRepository.findFirstByStatusOrderBySyncDateTimeDesc(CvsSyncStatus.SUCCESS))
    ...
    void recordsFailureWhenBuildingChangeSetFails() {
        ...
        when(changeSetBuilder.build(any(), any()))
                .thenThrow(new CvsConfigParseException("cannot parse config file"));
    ...
    void infrastructureFailureIsRetriedWithoutRecord() {
        doThrow(new CvsClientException("origin unreachable")).when(cvsClient).pull();
    ...
    void initializesTheClientOnlyOnce() {
        ...
        verify(cvsClient, times(1)).init();
        verify(cvsClient, times(2)).pull();
    }
    ...
    private CvsSyncLog log(String commitId) {
        return new CvsSyncLog(commitId, OffsetDateTime.now(), CvsSyncStatus.SUCCESS, null);
    }
}