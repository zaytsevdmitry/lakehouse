...
- **config-svc** - the source of configurations. Configuration changes are published to Kafka (topic `configuration_changes`) as `ConfigurationChangeDTO`, everything else is exposed via REST.
...
1. config-svc publishes configuration changes to Kafka (topic `configuration_changes`) as `ConfigurationChangeDTO` (kind `Schedule`, action `SAVE`, object `ScheduleEffectiveDTO`).
2. scheduler-svc consumes `ConfigurationChangeDTO`; when building a task instance it requests the effective task configuration (`TaskDTO`) from config-svc and creates the `ScheduleTaskInstance`.
